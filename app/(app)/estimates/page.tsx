...
import { createEstimateAction, reviewEstimateAction } from "@/lib/actions/estimate-actions";
import { requireUser } from "@/lib/auth";
import { db } from "@/lib/db";
import { formatMinutes } from "@/lib/utils";
import { canFullyModerateProject, isRoleScopedManager } from "@/lib/permissions";
import { getVisibleProjects } from "@/lib/queries";
import { getAwsApiBaseUrl, getEstimatesFromAws, type AwsEstimateListItem } from "@/lib/aws-api";
...
export default async function EstimatesPage({ searchParams }: { searchParams?: Promise<{ create?: string }> }) {
  ...
  const estimates = useAwsApi
    ? ((await getEstimatesFromAws()).items as AwsEstimateListItem[])
    : ((await db.estimate.findMany({
        ...estimateWithRelations,
        where:
          user.userType === "EMPLOYEE"
            ? { employeeId: user.id, projectId: { in: projects.length ? projects.map((p) => p.id) : ["__none__"] } }
            : user.userType === "TEAM_LEAD" || isRoleScopedManager(user)
              ? { employeeId: { in: assignedScopedEmployeeIds.length ? assignedScopedEmployeeIds : ["__none__"] }, projectId: { in: projects.length ? projects.map((p) => p.id) : ["__none__"] } }
              : { projectId: { in: projects.length ? projects.map((p) => p.id) : ["__none__"] } },
      })) as EstimateRow[]);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={canCreate ? <Link className="btn-primary" href="/estimates?create=1">Add Estimate</Link> : null}
      />
      ...
      {showCreate && canCreate ? (
        <form action={createEstimateAction} className="card p-6">
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            ...
            <div>
              <label className="label">Estimated minutes <span className="text-red-600">*</span></label>
              <input className="input" type="number" name="estimatedMinutes" min="15" step="15" required />
            </div>
            <div className="md:col-span-2">
              <label className="label">Notes</label>
              <textarea className="input min-h-28" name="notes" />
            </div>
            <div className="md:col-span-2"><button className="btn-primary w-full md:w-auto">Submit estimate</button></div>
          </div>
        </form>
      ) : null}
        ...
        <table className="table-base">
          ...
          <tbody className="divide-y divide-slate-100">
            {estimates.map((estimate) => {
              const canReview = canFullyModerateProject(user) || ((user.userType === "TEAM_LEAD" || isRoleScopedManager(user)) && managedIds.has(estimate.employeeId) && estimate.employee.functionalRole === user.functionalRole);
              ...
              return (
                <tr key={estimate.id}>
                  <td className="table-cell align-top">
                    ...
                    <div className="text-xs text-slate-500">{estimate.countryId ? countryMap.get(estimate.countryId) ?? "—" : "No specific country"}</div>
                    {latestReview?.remarks ? <div className="mt-2 rounded-lg bg-amber-50 px-2 py-1 text-xs text-amber-800">Review note: {latestReview.remarks}</div> : null}
                  </td>
                  ...
                  <td className="table-cell align-top">
                    <span className={estimate.status === "APPROVED" ? "badge-emerald" : estimate.status === "REJECTED" ? "badge-amber" : estimate.status === "REVISED" ? "badge-blue" : "badge-slate"}>{estimate.status}</span>
                  </td>
                  <td className="table-cell align-top">
                    {canReview && estimate.status === "SUBMITTED" ? (
                      <div className="flex flex-wrap gap-2">
                        ...
                        <form action={reviewEstimateAction}>
                          ...
                          <input type="hidden" name="comment" value="Please update and resubmit this estimate." />
                          <button className="btn-secondary !px-3 !py-1.5 text-xs">Revise</button>
                        </form>
                      </div>
                    ) : canResubmit ? (
                      <a href={`/estimates/${estimate.id}/edit`} className="text-sm font-medium text-blue-600 hover:underline">Edit &amp; Resubmit</a>
                    ) : (
            ...
            {estimates.length === 0 ? <tr><td className="table-cell text-center text-sm text-slate-500" colSpan={5}>No estimates found.</td></tr> : null}
          </tbody>
        </table>
      </div>
    </div>
